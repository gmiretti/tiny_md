...
def get_flags_str(cc, cflags, ldflags, paramflags, wflags):
    return 'CC="%s" CFLAGS="%s" LDFLAGS="%s" PARAMFLAGS="%s" WFLAGS="%s"' % (cc, cflags, ldflags, paramflags, wflags)
...
def build(
    ...
    with c.cd(src_dir):
        print(c.cwd)
        print("Building!")
        c.run("make tiny_md %s" % get_flags_str(cc, cflags, ldflags, paramflags, wflags))
        print("Built!")
    exec_dir = get_exec_dir(solution)
...
def run(
    ...
    stat_csv_path = log_dir / f"{get_log_prefix(exec_name)}-{metric}.csv"
    command = (f"perf stat -x '{CSV_SEPARATOR}' "
               "-e 'cs,migrations,faults' "
               f"-M '{metric}' "
               f"-r {retries} "
               f"-o {stat_csv_path} "
               f"{exec_path}"
    ...
    c.run(
        ...
        warn=True)
    c.run(
        f"mv thermo.log {log_dir / get_log_prefix(exec_name)}-thermo.log",
        warn=True)
